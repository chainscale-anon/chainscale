...
## Configuration (Chainboost.toml) Explained:

~~~~
Servers = 8 (Number of servers running the simul executable)
Simulation = "ChainBoost" (Leave Intact)
BF = 2 (Leave Intact)
Rounds = 100 (Ignore)
timeout = 12000000 (Timeouts see below)
RunWait = "12000000s" (Run wait: see below)
Suite = "bn256.adapter" (The elliptic curve used for CoSi signature (BN256))
FileSizeDistributionMean = 115 (The mean size of a file)
FileSizeDistributionVariance = 5 (The variance of a file)
ServAgrDurationDistributionMean = 40 (The mean duration of a contract in rounds)
ServAgrDurationDistributionVariance = 20 (the variance on the duration of a contract)
NegotiationDurationVariance = 9 (the variance on the negotiation of a contract)
NegotiationDurationMean = 3 (the duration of the variation of a contract)
PercentageTxPay = 30 (the percentage of the traffic that is TxPay)
PayPercentOfTransactions = 0.02 
NumberOfPayTXsUpperBound = 100
SectorNumber = 2 (Ignore)
NumberOfActiveContractsPerServer = 1 (the number of contracts served per serever)
SimulationRounds = 61 (number of mainchain rounds to run this simulations)
SimulationSeed = 100 (The randomness seed of the simulation)
NbrSubTrees = 1 (The number of subtress for CoSi communication - ignore)
CommitteeWindow = 50 (the size of the comittee)
Threshold = 33 (the Threshold on which to accept a signature - 33/50 = 66%)
StoragePaymentEpoch = 0 (pay right away - ignore)
MCRoundPerEpoch = 10 (the number of mainchain rounds per epoch, must be same as SCRoundDuration)
SCRoundPerEpoch = 30 (the number of sidechain rounds per epoch, must be same as MCRoundDuration)
MCRoundDuration = 30 (the number of sidechain rounds per epoch, must be same as SCRoundPerEpoch)
SCRoundDuration = 10 (the number of mainchain rounds per epoch, must be same as MCRoundPerEpoch)
SideChains = ["por", "marketmatch"] (the name of the sidechains supported by the system: choices are por, por2, por3, marketmatch, dispute)
Shardcount = 4 ("Number of shards in the system)
VMs = ["192.168.3.220", "192.168.3.221", "192.168.3.222", "192.168.3.223", "192.168.3.224", "192.168.3.225", "192.168.3.226", "192.168.3.227"] (VMs running the simul executable and running miners (count of which should be in Servers above))
Nodes = 1000 (number of miners in the system, must be same as Hosts)
FaultyContractsRate=0.1 (the percentage of contracts considered as faulty, to create dispute transactions)

Hosts, SimState, MainChainBlockSize, SideChainBlockSize 
1000, 1, 1000000, 1000000


Hosts: the number of miners in the network, must be same as Nodes.
SimState = 1 for mainchain only (or sharding), 2 for sidechains. [note that running 2 on sharded market won't work correctly]
MainchainBlockSize: mainchain blocksize in bytes
SidechainBlockSize: sidechain blocksize in bytes
~~~~

## Running the experiment locally

To run the experiment locally, use the following method:

build the project: `make`

copy `build/chainboost.toml` into `build/simul/{OS}/{arch}`

run: `./simul`

Note: the maximum Nodes/Hosts [On Mac Mini M2 Pro - 16 GiB of RAM] the simulation supports in local mode is 1000.

## Experimental setup architecture

The setup used to run the experiment is as follows:

![](experimental-setup.png)


The user can build the binaries in their local machine, then execute `make deploy USER=<user>` to upload the compiled binaries to the gateway using rsync.

[A User needs to specify the gateway domain in the makefile]

To run the experiments, the user needs to have first `orchestrator/ssh.toml` to specify the VMs in the field `Servers`, the files to upload to the VM in `Files`, the outputs to be collected by the orchestrator in `Outputs`, the user that is running the experiments in the VMs in user.

Once the orchestrator is run (on the gateway), it establishes ssh connections with the VMs specified in Servers, and runs the experiment, and collects the logs and the files specified in `Outputs` from each VM. 

### Notes:
  The DB files (for the first VM in the list) contain the results of the experimental run.
  The stdout.txt file contains the log trace of the miner (for all miners).


## On Building using Makefiles

this repo contains three Makefiles, one on the root of the repo, one in `simulation/manage/simulation/Makefile` and one in `simulation/platform/ResearchLab_users/Makefile`
