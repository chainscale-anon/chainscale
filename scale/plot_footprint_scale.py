...
def plot(dataframedict):
        ...
        fig.legend(loc="upper left", ncol=1, bbox_to_anchor=(0.15, 0.9), fontsize="xx-small")
        plt.grid()
...
if __name__ == "__main__":
    ...
    for mc, v in file_config.items():
        for sc in contacts.keys():
            ...
            print (f"\t mc: {mc_df["BCSize"].sum()/1e6}")
            mc_mean = mc_df["BCSize"].sum()
            for element in v:
                sc_df = get_pandas_df(f"{mc}/{sc}/{element}")[:-1]
                sc_df.rename(columns=sc_cols, inplace=True)
                sc_df = sc_df[sc_df["RoundNumber"] == 30]
                print (f"\t {sc} {element}: {sc_df["BCSize"].sum()/1e6}")
                mc_mean += sc_df["BCSize"].sum()
            dfdict[mc][0].append(contacts[sc])
            dfdict[mc][1].append(mc_mean/1e6)
    print(dfdict)
    plot(dfdict)